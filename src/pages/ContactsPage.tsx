    ...
    <section className="max-w-[1340px] mx-auto px-5 pb-12">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {OFFICES.map((office) => (
          ...
          >
            <h2 className="text-lg font-black text-slate-900 tracking-tight">{office.city}</h2>
            ...
            <div className="space-y-1.5">
              {office.phones.map((phone) => (
                <div key={phone.href} className="flex items-center gap-2.5 text-sm">
                  ...
                  <a
                    href={phone.href}
                    className="font-bold text-slate-900 hover:text-brand-blue transition-colors whitespace-nowrap"
                  >
                    {phone.label}
                  </a>
                  {phone.note && <span className="text-xs text-slate-400">({phone.note})</span>}
                </div>
              ))}
            </div>
      ...
      <div className="mt-6 bg-blue-50 border border-blue-100 rounded-2xl p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <span className="text-xs text-slate-500">ООО «ИНВИТ» · EUROBAND — собственная торговая марка</span>
      </div>
    </section>
