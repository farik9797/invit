...
export const ProductCatalogSection: React.FC<ProductCatalogSectionProps> = ({
  ...
  return (
    <section id="catalog" className="py-12 bg-white border-b border-slate-200">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between mb-8 gap-4">
          <div>
            <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-[#0B5FA5] mb-1">
              ...
              <span>B2B Интернет-Каталог Продукции</span>
            </div>
          ...
          <div className="flex items-center gap-1.5 bg-slate-100 p-1.5 rounded-xl border border-slate-200 overflow-x-auto">
            {selectedCategorySlug && (
              <button
                ...
                className="text-xs font-bold px-3 py-2 rounded-lg bg-red-100 text-red-700 hover:bg-red-200 transition-colors flex items-center gap-1 shrink-0"
              >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {displayedProducts.map((product) => {
            ...
            return (
              <div
                key={product.id}
                className="bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-xl hover:border-blue-300 transition-all duration-300 flex flex-col justify-between overflow-hidden group"
              >
                {/* Image & Badge Header */}
                <div className="relative h-48 bg-slate-100 overflow-hidden flex items-center justify-center p-4">
                  ...
                  {/* Badge */}
                  {product.badge && (
                    <span className="absolute top-2.5 left-2.5 bg-[#F39200] text-slate-950 font-black text-[10px] uppercase px-2 py-0.5 rounded shadow">
                      {product.badge}
                    </span>
                  )}

                  {/* Article Tag */}
                  <span className="absolute top-2.5 right-2.5 bg-slate-900/80 backdrop-blur text-white font-mono text-[10px] font-bold px-2 py-0.5 rounded">
                    {product.code}
                  </span>
                ...
                <div className="p-4 flex-1 flex flex-col justify-between space-y-3">
                  <div>
                    {/* Category Label */}
                    <div className="text-[11px] font-semibold text-[#0B5FA5] uppercase tracking-wider mb-1">
                    ...
                    <h3
                      onClick={() => onSelectProduct(product)}
                      className="text-sm font-bold text-slate-900 line-clamp-2 hover:text-[#0B5FA5] cursor-pointer transition-colors leading-snug"
                    >
                      {product.title}
                    </h3>
                  ...
                  <div className="bg-slate-50 p-2.5 rounded-lg border border-slate-200/80 text-[11px] text-slate-700 space-y-1">
                    {product.specs.width && (
                      <div className="flex justify-between">
                        ...
                        <span className="font-semibold text-slate-900">{product.specs.width}</span>
                      </div>
                    )}
                    {product.specs.length && (
                      <div className="flex justify-between">
                        <span className="text-slate-400">Намотка:</span>
                        <span className="font-semibold text-slate-900">{product.specs.length}</span>
                      </div>
                    )}
                    {product.specs.material && (
                      <div className="flex justify-between">
                        <span className="text-slate-400">Основа:</span>
                        <span className="font-semibold text-slate-900 truncate max-w-[140px]">{product.specs.material}</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 pt-1">
                    <button
                      onClick={() => onSelectProduct(product)}
                      className="bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold text-xs py-2 px-2 rounded-lg transition-colors flex items-center justify-center gap-1 cursor-pointer"
                    >
                    ...
                    <button
                      ...
                      className={`font-bold text-xs py-2 px-2 rounded-lg transition-all flex items-center justify-center gap-1 cursor-pointer shadow-sm ${
                        isAdded
