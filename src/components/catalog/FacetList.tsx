...
/*
 * Список значений признака с флажками и счётчиками — общая начинка для двух
 * оболочек: свёрнутого блока в выдвижной панели на телефоне и выпадающего
 * окошка в строке фильтров на широком экране.
 *
 * Брендов шестьдесят, поэтому показываем восемь самых частых, а длинные списки
 * получают собственный поиск: искать «Макиту» прокруткой — занятие на полминуты.
 */
...
export const FacetList: React.FC<FacetListProps> = ({
  ...
  return (
    <>
      ...
      {!found.length && <p className="py-2 text-[13px] text-inv-ink-muted">Ничего не нашлось</p>}
