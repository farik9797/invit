          ...
          {submitted ? (
            <div className="py-10 text-center space-y-4">
              ...
              <button
                ...
                className="bg-brand-blue text-white font-semibold text-sm px-6 py-2.5 rounded-lg"
              >
                Очистить и закрыть
              </button>
            </div>
            ...
            <>
              ...
              <div className="divide-y divide-line border border-line rounded-xl overflow-hidden">
                {items.map(({ product, selectedWidth, quantity }) => (
                  <div key={product.id} className="p-3.5 flex items-center justify-between gap-3 hover:bg-surface-soft text-xs">
                    <img
                      src={product.image}
                      alt={product.title}
              ...
              <form onSubmit={handleSubmitQuote} className="space-y-3 bg-surface-soft p-4 rounded-xl border border-line text-xs">
                ...
                <button
                  type="submit"
                  className="w-full bg-brand-red hover:bg-brand-red-hover text-white font-bold py-3 px-4 rounded-xl shadow-xs transition-all flex items-center justify-center gap-2 text-xs uppercase cursor-pointer"
                >
