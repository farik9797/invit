        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {advantages.map((adv, idx) => {
            ...
            return (
              <div
                key={idx}
                className="p-4 rounded-xl bg-slate-50 hover:bg-blue-50/50 border border-slate-200/80 hover:border-blue-200 transition-all duration-200 flex items-start gap-3.5 group shadow-sm"
              >
                <div className="p-3 bg-white rounded-lg border border-slate-200 shadow-sm shrink-0 group-hover:scale-105 transition-transform">
                  <Icon className={`w-6 h-6 ${adv.color}`} />
                </div>
                <div className="space-y-1">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <h3 className="text-sm font-bold text-slate-900 group-hover:text-[#0B5FA5] transition-colors leading-snug">
                      {adv.title}
                    </h3>
                  </div>
