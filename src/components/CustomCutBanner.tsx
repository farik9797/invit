...
export const CustomCutBanner: React.FC<CustomCutBannerProps> = ({ onOpenCallbackWithNote }) => {
  return (
    <section className="py-14 sm:py-16 bg-gradient-to-r from-slate-950 via-[#08487F] to-slate-950 text-white border-y border-blue-900/80 relative overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-10 bg-[radial-gradient(#D91920_1px,transparent_1px)] [background-size:16px_16px]"></div>
      ...
      <div className="max-w-[1340px] mx-auto px-5 relative z-10">
        <div className="flex flex-col lg:flex-row items-center justify-between gap-8">
          ...
          <div className="space-y-4 max-w-2xl">
            <div className="inline-flex items-center gap-2 bg-brand-red text-white font-black text-xs uppercase px-3 py-1 rounded-md shadow-xs">
              <Scissors className="w-4 h-4" />
              <span>Цех высокоточной порезки EUROBAND</span>
            </div>

            <h2 className="text-2xl sm:text-3xl lg:text-4xl font-black tracking-tight leading-tight">
              Производим клейкие ленты <span className="text-brand-red-light">нетипичных размеров</span> под заказ
            </h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
              <div className="flex items-center gap-2.5 text-xs sm:text-sm text-slate-200 font-bold">
                <CheckCircle className="w-4 h-4 text-brand-red-light shrink-0" />
          ...
          <div className="shrink-0 w-full sm:w-auto">
            <button
              ...
              className="w-full sm:w-auto bg-brand-red hover:bg-brand-red-hover active:scale-98 text-white font-black py-4 px-8 rounded-2xl shadow-xl transition-all flex items-center justify-center gap-2.5 text-xs sm:text-sm uppercase tracking-wider cursor-pointer"
            >
